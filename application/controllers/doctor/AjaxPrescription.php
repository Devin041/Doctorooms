	...
	public function create()
	{
		...
		if (!empty($check_data)) {
			...
			if (empty($check_prescription)) {
				...
				$dgno_table= $this->Table_master_model->get_all_table_master($this->user_data->id);
				$prescription_id = $check_prescription->id;
			...
			$this->view_data['labels'] = $this->Label_model->get_all_label_master($this->user_data->id);
			...
			$this->view_data['_view'] = "doctor/AjaxPrescription/index";
...
foreach ($medicine_list as $md){
	...
	public function get_saggestion()
	{
		...
		$this->view_data['questions']=$questions;
		 ...
		 $this->load->view('doctor/AjaxPrescription/get_saggestion_ajax_view',$this->view_data);
	}
