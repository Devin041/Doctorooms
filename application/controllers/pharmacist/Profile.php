...
class Profile extends Pharmacist_contoller
{
	...
	public function index()
	{
		...
		$u_data = $this->pharmacist->get_by_user_id($this->user_data->id);
		...
		$this->view_data['_view'] = "pharmacist/profile";
	...
	public function update()
	{
		if ($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			if ($this->form_validation->run("pharmacist_profile")) {
				/**
				...
				$this->pharmacist->update($this->user_data->id, $forRec);
				$this->user->update($this->user_data->id, $forUser);

				$this->_alertSuccessResponce(true, 'Profile Updated', 'Profile Not Updated', 'pharmacist/profile');
			} else {
				$this->view_data['profile'] = $this->user->get_by_id($this->user_data->id);
				$u_data = $this->pharmacist->get_by_user_id($this->user_data->id);
				unset($u_data->password);
	...
	public function change_pass()
	{
		...
		if ($this->form_validation->run("change_password")) {
			...
			return $this->_alertSuccessResponce($this->user->update($this->user_data->id, $upData), "Your Password is changed", "Your Password is not changed", "pharmacist/profile");
		} else {
			$this->view_data['profile'] = $this->user->get_by_id($this->user_data->id);
			$u_data = $this->pharmacist->get_by_user_id($this->user_data->id);
			unset($u_data->password);
	...
	public function update_profile_photo()
	{
		if (!empty($_FILES['profile']['name'])) {
			...
			if ($profData['status'] == 'true') {
				...
				return $this->_alertSuccessResponce($this->user->update($this->user_data->id, $imgData), "Your Profile is updated", "Your profile is not updated", "pharmacist/profile");
			} else {
				return $this->_alertSuccessResponce(FALSE, "Imgs", $profData['upload_error'], "pharmacist/profile");
			}
		} else {
			return $this->_alertSuccessResponce(FALSE, "Imgs", "Please choose image", "pharmacist/profile");
		}
	}
