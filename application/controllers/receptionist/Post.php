...
class Post extends Reception_contoller
{
    ...
    public function index()
    {
        ...
        $this->view_data['_view'] = "receptionist/post/index";
        $this->load->view("layouts/master_page_receptionist", $this->view_data);
    }

    public function create()
    {
        $this->view_data['_view'] = "receptionist/post/add";
            ...
            if ($this->form_validation->run("blog")) {
                if (!empty($_FILES['image']['name'])) {
                    ...
                    if ($bgImgUp['status'] == "true") {
                        ...
                        $this->_alertSuccessResponce($this->post->create($post_data), "Post Created", "Post not created", "receptionist/post");
                    } else {
    ...
    public function edit($id)
    {
        ...
        if (is_null($blogData)) {
            redirect("receptionist/blog");
        }

        $this->view_data['post'] = $blogData;
        $this->view_data['_title'] = "Edit Post";
        $this->view_data['_view'] = "receptionist/post/edit";
        $this->load->view("layouts/master_page_receptionist", $this->view_data);
    }
            ...
            if ($this->form_validation->run("blog")) {
                ...
                if (!empty($_FILES['image']['name'])) {
                    ...
                    if ($bgImgUp['status'] == "true") {
                        ...
                        $this->_alertInfoResponce($this->post->update($id, $post_data), "Post updated", "Post not updated", "receptionist/post");
                    } else {
                        $this->view_data['imgError'] = $bgImgUp['upload_error'];
                        $this->view_data['post'] = $blogData;
                        $this->view_data['_title'] = "Edit Post";
    ...
    public function delete($id)
    {
        ...
        $this->_alertWarningResponce($this->post->destroy($id, $this->user_data->id), "Post deleted", "Post not deleted", "receptionist/post");
    }
    ...
    public function hide_unhide($id)
    {
        ...
        $this->_alertInfoResponce($this->post->update($id, $data), "Status changed", "Status not changed", "receptionist/post");
    }
}